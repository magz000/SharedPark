...
public class TabSharedParking extends Fragment {
    ...
    private void GetOfflineData() {
        ...
        /*if (ReservationItem.count(ReservationItem.class) > 0) {
            //  reservationItemList = ReservationItem.findWithQuery(ReservationItem.class, "Select * from reservation_item where status in (? , ?) and restype = ?","No Show", "Out", ReservationsFragment.res_type);
            List<ReservationItem> reservationItemList = ReservationItem.findWithQuery(ReservationItem.class, "Select * from reservation_item where status in (?,?) ORDER BY timein asc", "Arrived", "Overdue");
            ReservationRecyclerviewAdapter adapter = new ReservationRecyclerviewAdapter(getActivity(), reservationItemList, "history", preferences.getString("webservice_root", WebServiceHelper.web_tip));
            mRecyclerView.setAdapter(adapter);
            if (reservationItemList.isEmpty()) {
                no_reservation.setVisibility(View.VISIBLE);
            }
